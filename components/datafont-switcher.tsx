...
import {  Check, ChevronDownIcon, ScanSearch } from 'lucide-react'
...
export function DataFontSwitcher<T extends { id: string; name: string }>({
  // selected,
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger className={`${className} `} asChild>
          <Button
            ...
            role="combobox"
            aria-expanded={open}
            aria-label={`Select a ${'Fonte de dados'}`}
            className={'  md:flex md:px-2 md:h-[34px] rounded-2xl'}
          >
            <ScanSearch/>
            {windowWidth > 768 && (
              <>
                {selectedSubject?.name || `  ${'Fonte de dados'}`}
                <ChevronDownIcon />
              </>
            )}
        ...
        <PopoverContent className="w-[200px] p-0">
          <Command>
            <CommandInput
              placeholder={`Buscar ${'Fonte de dados'.toLowerCase()}...`}
            />
            <CommandList>
              <CommandEmpty>
                Nenhuma {'Fonte de dados'.toLowerCase()} encontrada.
              </CommandEmpty>
              <CommandGroup heading={'Fonte de dados'}>
                {subjects?.map((item) => (
                  <CommandItem
                    key={item.id}
                    onSelect={() => {
